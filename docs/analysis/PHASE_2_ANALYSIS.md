...
- **폼(Form)**: React 컴포넌트에서 사용자 입력을 관리하는 UI 요소. 코드에서는 `form` 변수로 표현되며, React Hook Form 라이브러리를 사용합니다.
- **필드(Field)**: 폼 내의 개별 입력 요소 (예: chainName, network, seedPhrase, awsAccessKey 등). 각 필드는 유효성 검사 규칙을 가집니다.
- **에러(Error)**: 유효성 검사 실패 (형식 오류, 필드 누락) 또는 API 호출 실패를 의미합니다.
- **응답(Response)**: 백엔드에서 클라이언트로 반환하는 HTTP 응답 객체. 성공/실패 상태, 메시지, 데이터를 포함합니다.
- **Preset**: 사전 정의된 배포 템플릿 (general, defi, gaming, full). 사용자가 기본 설정을 선택하면 나머지 파라미터는 자동으로 채워집니다.
- **Override**: 사용자가 Preset의 기본값을 수정하는 것. 특정 필드값을 변경할 때 사용됩니다.
...
```json
{
  ...
  "chainName": "my-chain",
  "network": "Testnet",
  "seedPhrase": "word1 word2 word3 word4 word5 word6 word7 word8 word9 word10 word11 word12",  // 12개 공백으로 구분된 단어
  "infraProvider": "aws",
...
### 참고: Backend entities.Response 구조

**파일**: `/Users/theo/workspace_tokamak/trh-backend/pkg/entities/response.go`

백엔드의 모든 HTTP 응답은 다음의 `entities.Response` 구조를 사용합니다:

```go
// Backend entities.Response 구조
...
**필드 설명**:
- `Status`: HTTP 상태 코드 (200, 400, 500 등)
- `Message`: 사람이 읽을 수 있는 메시지 ("ok", 에러 설명 등)
- `Data`: 응답 데이터 (선택사항, omitempty 태그로 null일 때 생략)
...
&entities.Response{
  ...
  Message: "awsAccessKey, awsSecretKey, and awsRegion are required for aws provider",
}
```
...
### ⚠️ [CRITICAL - PRODUCTION BUG] Backend PresetDeploy 핸들러 에러 처리 버그

**이것은 현재 프로덕션 코드에 존재하는 버그입니다. 즉시 수정이 필요합니다.**
