...
func LoadArtifacts() (*Artifacts, error) {
	mips, err := foundry.ReadArtifact("../../packages/tokamak/contracts-bedrock/forge-artifacts/MIPS.sol/MIPS.json")
	if err != nil {
		return nil, fmt.Errorf("failed to load MIPS contract: %w", err)
	}

	oracle, err := foundry.ReadArtifact("../../packages/tokamak/contracts-bedrock/forge-artifacts/PreimageOracle.sol/PreimageOracle.json")
	if err != nil {
