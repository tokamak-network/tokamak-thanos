...
func NewL1Genesis(config *DeployConfig) (*core.Genesis, error) {
	...
	// Decide initial blob gas fields based on Cancun activation at genesis
	isCancunActive := chainConfig.CancunTime != nil && *chainConfig.CancunTime <= uint64(timestamp)
	var excessBlobGasPtr *uint64
	var blobGasUsedPtr *uint64
	if isCancunActive {
		excessBlobGasPtr = u64ptr(0)
		blobGasUsedPtr = u64ptr(0)
	} else {
		excessBlobGasPtr = (*uint64)(config.L1GenesisBlockExcessBlobGas)
		blobGasUsedPtr = (*uint64)(config.L1GenesisBlockBlobGasUsed)
	}
	...
	return &core.Genesis{
		...
		BaseFee:       baseFee.ToInt(),
		ExcessBlobGas: excessBlobGasPtr,
		BlobGasUsed:   blobGasUsedPtr,
		Alloc:         map[common.Address]types.Account{},
	}, nil
}
