	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			indexedBlobHash := eth.IndexedBlobHash{
				Index: tc.blobIndex,
				Hash:  eth.KZGToVersionedHash(kzg4844.Commitment(blobCommitment)),
			}
...
func TestInitRootsOfUnity(t *testing.T) {
	...
	for i := 0; i < params.BlobTxFieldElementsPerBlob; i++ {
		...
		kzgProof, evaluation, err := kzg4844.ComputeProof((*kzg4844.Blob)(blob), z)
		require.NoError(t, err)
		require.Equal(t, targetFieldElement[:], evaluation[:])

		// Verify the proof for good measure
		err = kzg4844.VerifyProof(kzg4844.Commitment(blobCommitment), z, targetFieldElement, kzgProof)
		require.NoError(t, err)
	}
}
