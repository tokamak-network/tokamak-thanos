...
func TestPrestateProviderCache_CreateAndCache(t *testing.T) {
	cache := NewPrestateProviderCache(nil, "", func(ctx context.Context, prestateHash common.Hash) (types.PrestateProvider, error) {
	...
	provider2, err := cache.GetOrCreate(ctx, hash2)
	require.NoError(t, err)
