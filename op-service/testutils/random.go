...
func RandomBlob(rng *rand.Rand) (*eth.Blob, eth.Bytes48, error) {
	var blob eth.Blob
	rng.Read(blob[:])
	// Zero out the top bits of each field element to make it a valid BLS field element
	for i := 0; i < 4096; i++ {
		blob[i*32] = 0
	}
	// Compute the actual KZG commitment from the blob
	c, err := blob.ComputeKZGCommitment()
	if err != nil {
		return nil, eth.Bytes48{}, err
	}
	return &blob, eth.Bytes48(c), nil
}
