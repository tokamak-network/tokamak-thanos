...
// functionality.
func TestETHBridge(t *testing.T) {
	e2e.InitParallel(t)
	...
	// compute the deposit transaction hash + poll for it
	portal, err := bindingspreview.NewOptimismPortal2(cfg.L1Deployments.OptimismPortalProxy, l1Client)
	require.NoError(t, err)
	...
	provedReceipt, finalizedReceipt, _, _ := e2e.ProveAndFinalizeWithdrawal(t, cfg, sys, "sequencer", cfg.Secrets.Bob, withdrawalReceipt)
	require.Equal(t, types.ReceiptStatusSuccessful, provedReceipt.Status)
	require.Equal(t, types.ReceiptStatusSuccessful, finalizedReceipt.Status)
