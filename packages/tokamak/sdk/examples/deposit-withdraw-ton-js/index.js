...
const l1Provider = new ethers.providers.StaticJsonRpcProvider(process.env.L1_URL)
const l2Provider = new ethers.providers.StaticJsonRpcProvider(process.env.L2_URL)
...
const withdrawTON = async (amount) => {
  ...
  console.log("withdrawal Tx:", withdrawalTx.transactionHash, " Block", withdrawalTx.blockNumber, " hash", withdrawal.hash)

  l2Balance = await l2Wallet.getBalance()
  console.log("Updated l2 native balance:", l2Balance.toString())

  // // Check ready for prove
  await messenger.waitForMessageStatus(withdrawalTx.transactionHash, optimism.MessageStatus.READY_TO_PROVE)
  ...
  const finalizeInterval = setInterval(async () => {
    const currentStatus = await messenger.getMessageStatus(withdrawalTx.transactionHash)
