...
import { Claim, Duration, GameType, Hash, GameTypes, Proposal, OutputRoot } from "src/dispute/lib/Types.sol";
...
abstract contract OPContractsManagerBase {
    ...
    // Super game encoder functions are not used in Tokamak
    ...
    {
        // Tokamak only supports standard games (CANNON, PERMISSIONED_CANNON)
        // which always have a valid L2 chain ID
...
contract OPContractsManagerGameTypeAdder is OPContractsManagerBase {
        ...
        for (uint256 i = 0; i < _gameConfigs.length; i++) {
            ...
            // Separate context to avoid stack too deep.
            {
                ...
                if (gameConfig.disputeGameType.raw() == GameTypes.CANNON.raw()) {
                    ...
                    // Super game types are not supported in Tokamak
    ...
    function updatePrestate(OPContractsManager.OpChainConfig[] memory _prestateUpdateInputs) public {
        ...
        for (uint256 i = 0; i < _prestateUpdateInputs.length; i++) {
            ...
            // Tokamak only supports CANNON and PERMISSIONED_CANNON
...
contract OPContractsManagerUpgrader is OPContractsManagerBase {
        ...
        virtual
    {
        ...
        // NOTE: Tokamak's SuperchainConfig doesn't have an upgrade() function yet
        // Uncomment when SuperchainConfig.upgrade() is implemented
        /*
        ...
        */
        ...
        for (uint256 i = 0; i < _opChainConfigs.length; i++) {
            ...
            // NOTE: Tokamak's SystemConfig doesn't have an upgrade() function yet
            // Uncomment when SystemConfig.upgrade() is implemented
            /*
            upgradeToAndCall(
            ...
            */
            ...
            IAnchorStateRegistry newAnchorStateRegistryProxy;
            {
                ...
                // Separate context to avoid stack too deep.
                {
                    ...
                    // NOTE: Tokamak uses OutputRoot instead of Proposal
                    upgradeToAndCall(
                        _opChainConfigs[i].proxyAdmin,
            ...
            // Separate context to avoid stack too deep.
            {
                ...
                // NOTE: Tokamak's OptimismPortal doesn't have an upgrade() function yet
                // Uncomment when OptimismPortal.upgrade() is implemented
                ...
                // NOTE: Tokamak's OptimismPortal doesn't have migrateLiquidity() yet
            ...
            // Separate context to avoid stack too deep.
            {
                ...
                // NOTE: Tokamak's bridge contracts don't have upgrade() functions yet
                // Uncomment when upgrade() functions are implemented
                /*
...
contract OPContractsManagerDeployer is OPContractsManagerBase {
        ...
        returns (OPContractsManager.DeployOutput memory)
    {
        ...
        if (_input.deployRAT) {
            ...
            // NOTE: Tokamak's DisputeGameFactory doesn't have setRAT() yet
    ...
    {
        // Convert ResourceMetering.ResourceConfig to IResourceMetering.ResourceConfig
        ResourceMetering.ResourceConfig memory defaultConfig = Constants.DEFAULT_RESOURCE_CONFIG();
        resourceConfig_ = IResourceMetering.ResourceConfig({
            maxResourceLimit: defaultConfig.maxResourceLimit,
            elasticityMultiplier: defaultConfig.elasticityMultiplier,
            baseFeeMaxChangeDenominator: defaultConfig.baseFeeMaxChangeDenominator,
            minimumBaseFee: defaultConfig.minimumBaseFee,
            systemTxMaxGas: defaultConfig.systemTxMaxGas,
            maximumBaseFee: defaultConfig.maximumBaseFee
        });
    ...
    function encodeOptimismPortalInitializer(OPContractsManager.DeployOutput memory _output)
        ...
        returns (bytes memory)
    {
        // Tokamak's OptimismPortal.initialize() takes 2 parameters (no ethLockboxProxy)
        return abi.encodeCall(
            IOptimismPortal.initialize,
            (_output.systemConfigProxy, _output.anchorStateRegistryProxy)
        );
    }
...
/// @dev NOT USED IN TOKAMAK - Super games are not supported
/*
...
contract OPContractsManager is ISemver {
    ...
    // OPContractsManagerInteropMigrator is not used in Tokamak (Super games not supported)
    ...
    constructor(
        ...
        // OPContractsManagerInteropMigrator _opcmInteropMigrator,  // Not used in Tokamak
        ...
        // _opcmDeployer.assertValidContractAddress(address(_opcmInteropMigrator));  // Not used in Tokamak
        ...
        // opcmInteropMigrator = _opcmInteropMigrator;  // Not used in Tokamak
