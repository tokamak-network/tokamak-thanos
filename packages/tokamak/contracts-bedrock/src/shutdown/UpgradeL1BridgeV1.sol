...
contract UpgradeL1BridgeV1 is L1StandardBridge {
    ...
    /// @dev Uses delegatecall to retrieve owner from proxy storage slot
    modifier onlyOwner() {
    ...
    /// @param _address Recipient address
    function claim(address _position, string calldata _hash, address _token, uint256 _amount, address _address)
        internal
    {
    ...
    function getProxyImplementation() external view returns (address implementation) {
        assembly {
