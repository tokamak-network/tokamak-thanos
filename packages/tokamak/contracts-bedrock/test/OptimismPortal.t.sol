...
import { Portal_Initializer, SuperchainConfig_Initializer, CommonTest, NextImpl } from "test/CommonTest.t.sol";
...
contract OptimismPortal_Test is Portal_Initializer {
    ...
    function test_depositTransaction_smallGasLimit_reverts() external {
        ...
        op.depositTransaction({ _to: address(1), _mint: 0, _value: 0, _gasLimit: 0, _isCreation: false, _data: hex"" });
    }
    ...
    function test_depositTransaction_noValueEOA_succeeds() external {
        ...
        op.depositTransaction(NON_ZERO_ADDRESS, ZERO_VALUE, ZERO_VALUE, NON_ZERO_GASLIMIT, false, NON_ZERO_DATA);
    }
    ...
    function test_depositTransaction_withNativeTokenValueFromEOA_succeeds() external {
        ...
        vm.prank(address(this), address(this));
        op.depositTransaction(NON_ZERO_ADDRESS, NON_ZERO_VALUE, NON_ZERO_VALUE, NON_ZERO_GASLIMIT, false, NON_ZERO_DATA);
    ...
    function test_depositTransaction_withEthValueAndContractContractCreation_succeeds() external {
        ...
        vm.prank(address(this), address(alice));
        op.depositTransaction(ZERO_ADDRESS, NON_ZERO_VALUE, NON_ZERO_VALUE, NON_ZERO_GASLIMIT, true, NON_ZERO_DATA);
        assertEq(token.balanceOf(address(op)), NON_ZERO_VALUE);
    }
