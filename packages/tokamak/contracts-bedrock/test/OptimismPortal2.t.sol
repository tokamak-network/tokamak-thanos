...
import { Portal2_Initializer, SuperchainConfig_Initializer, CommonTest, NextImpl } from "test/CommonTest.t.sol";
...
contract OptimismPortal2_Deposit_Test is Portal2_Initializer {
    address depositor;
    ...
    function test_initialize_succeeds() external virtual {
        ...
        assertEq(optimismPortal2.guardian(), guardian);
        assertEq(address(optimismPortal2.superchainConfig()), address(sc));
    ...
    function testFuzz_receive_reverts(uint256 _value) external {
        ...
        (bool s,) = address(optimismPortal2).call(hex"");
        ...
        external
    {
        ...
        vm.prank(address(depositor), address(depositor));
        token.faucet(_mint);
        vm.prank(address(depositor), address(depositor));
...
contract OptimismPortal2_FinalizeWithdrawal_Test is Portal2_Initializer {
    ...
    function test_finalizeWithdrawalTransaction_provenWithdrawalHash_succeeds() external {
        uint256 bobAllowanceBalanceBefore = token.allowance(address(optimismPortal2), address(bob));
        ...
        assert(token.allowance(address(optimismPortal2), address(bob)) == bobAllowanceBalanceBefore + 100);
    }
    ...
    function test_finalizeWithdrawalTransaction_delayEdges_succeeds() external {
        ...
        // Attempt to finalize the withdrawal transaction 1 second before the proof has matured. This shoul fail.
...
contract OptimismPortal2_Upgradeable_Test is Portal2_Initializer {
...
contract OptimismPortal2_ResourceFuzz_Test is Portal2_Initializer {
        ...
        external
    {
        ...
        token.faucet(0x40);
        token.approve(address(optimismPortal2), 0x40);
