...
contract PreBridgeETH is Bridge_Initializer {
    ...
    function _preBridgeETH(bool isLegacy) internal {
        ...
        vm.expectCall(
            ...
            0,
            abi.encodeWithSelector(
                OptimismPortal.depositTransaction.selector, address(L2Messenger), 0, 0, baseGas, false, innerMessage
            )
        );
...
contract L1StandardBridge_DepositNativeToken_Test is Bridge_Initializer {
    ...
    function test_depositNativeToken_succeeds() external {
        ...
        vm.expectCall(
            address(op),
            abi.encodeWithSelector(
                OptimismPortal.depositTransaction.selector, address(L2Messenger), 100, 100, baseGas, false, innerMessage
            )
        );
