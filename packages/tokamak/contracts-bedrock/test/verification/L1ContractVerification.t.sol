...
contract L1ContractVerificationTest is Test {
    ...
    function setUp() public {
        ...
        l1StandardBridgeProxy = new L1ChugSplashProxy(address(l1StandardBridgeImpl));

        l1CrossDomainMessengerProxy = new LegacyProxy(address(l1CrossDomainMessengerImpl), address(mockProxyAdmin));

        optimismPortalProxy = new TransparentUpgradeableProxy(address(optimismPortalImpl), address(mockProxyAdmin), "");

        // Now deploy SystemConfig implementation with the proxy addresses
        systemConfigImpl = new MockSystemConfig();

        systemConfigProxy = new TransparentUpgradeableProxy(address(systemConfigImpl), address(mockProxyAdmin), "");

        // Set up the mock ProxyAdmin
        mockProxyAdmin.setImplementation(address(systemConfigProxy), address(systemConfigImpl));
        mockProxyAdmin.setImplementation(address(l1StandardBridgeProxy), address(l1StandardBridgeImpl));
        mockProxyAdmin.setImplementation(address(l1CrossDomainMessengerProxy), address(l1CrossDomainMessengerImpl));
        mockProxyAdmin.setImplementation(address(optimismPortalProxy), address(optimismPortalImpl));

        mockProxyAdmin.setAdmin(address(systemConfigProxy), address(mockProxyAdmin));
        mockProxyAdmin.setAdmin(address(l1StandardBridgeProxy), address(mockProxyAdmin));
        mockProxyAdmin.setAdmin(address(l1CrossDomainMessengerProxy), address(mockProxyAdmin));
        mockProxyAdmin.setAdmin(address(optimismPortalProxy), address(mockProxyAdmin));
        ...
        // Set the owner to the caller (thirdOwner) since the proxy initialization sets owner to proxy address
        MockSystemConfig config = MockSystemConfig(address(systemConfigProxy));
        config.setOwner(thirdOwner);

        // Verify the owner was set correctly
        require(config.owner() == thirdOwner, "Owner not set correctly");
        ...
        verifierProxyAdmin.transferOwnership(owner);

        // The initialization data
        bytes memory verifierInitData =
            abi.encodeWithSelector(L1ContractVerification.initialize.selector, nativeToken, thirdOwner);

        // Deploy the proxy with the implementation
        verifierProxy =
            new TransparentUpgradeableProxy(address(verifierImpl), address(verifierProxyAdmin), verifierInitData);
        ...
        // Grant ADMIN_ROLE to both owner and thirdOwner so they can perform admin operations
        verifier.addAdmin(owner);
        verifier.addAdmin(thirdOwner);

        // Grant DEFAULT_ADMIN_ROLE to owner so they can grant roles to others
    ...
    function testVerifyL1ContractsSuccess() public {
        ...
        verifier.setLogicContractInfo(address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)));
        ...
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)), address(safeWallet), "Test Rollup"
        );
    ...
    function testVerifyAndRegisterRollupConfig() public {
        ...
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)), address(safeWallet), "TestRollup"
        );
    ...
    function testVerifyL1ContractsFailInvalidProxyAdminContract() public {
        ...
        // Grant ADMIN_ROLE to the owner (deployer)
        verifier.addAdmin(owner);
        ...
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(differentProxyAdmin)), address(differentSafe), "Test Rollup"
        );
    ...
    function testVerifyL1ContractsFailInvalidProxyAdminAddress() public {
        ...
        MockProxyAdmin differentProxyAdmin = new MockProxyAdmin(owner);
        // Set the owner of the different proxy admin to a different safe wallet
        differentProxyAdmin.setOwner(makeAddr("differentSafeWallet"));

        // Set implementations for the different proxy admin
        differentProxyAdmin.setImplementation(address(systemConfigProxy), address(systemConfigImpl));
        differentProxyAdmin.setImplementation(address(l1StandardBridgeProxy), address(l1StandardBridgeImpl));
        differentProxyAdmin.setImplementation(address(l1CrossDomainMessengerProxy), address(l1CrossDomainMessengerImpl));
        differentProxyAdmin.setImplementation(address(optimismPortalProxy), address(optimismPortalImpl));

        // Set admin relationships for each proxy
        differentProxyAdmin.setAdmin(address(systemConfigProxy), address(differentProxyAdmin));
        differentProxyAdmin.setAdmin(address(l1StandardBridgeProxy), address(differentProxyAdmin));
        differentProxyAdmin.setAdmin(address(l1CrossDomainMessengerProxy), address(differentProxyAdmin));
        differentProxyAdmin.setAdmin(address(optimismPortalProxy), address(differentProxyAdmin));

        vm.stopPrank();

        vm.startPrank(thirdOwner);

        vm.expectRevert(L1ContractVerification.SafeWalletAddressMismatch.selector);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(differentProxyAdmin)), address(safeWallet), "Test Rollup"
        );
    ...
    function testVerifyL1ContractsFailMismatchedSafeWalletAddress() public {
        ...
        // Grant ADMIN_ROLE to the user so they can call the verification function
        ...
        // Verification should fail because user is not the deployer of the system config proxy
        vm.expectRevert(L1ContractVerification.CallerNotDeployer.selector);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)), address(safeWallet), "Test Rollup"
        );
    ...
    function testVerifyL1ContractsFailInvalidSystemConfig() public {
        ...
        mockProxyAdmin.setImplementation(address(systemConfigProxy), address(differentSystemConfigImpl));
     ...
     */
    function testVerifyL1ContractsFailWrongSystemConfigProxyWithCorrectImplementation() public {
        vm.startPrank(thirdOwner);
    ...
    function testVerifyL1ContractsFailInvalidL1StandardBridge() public {
        ...
        mockProxyAdmin.setImplementation(address(l1StandardBridgeProxy), address(differentL1StandardBridgeImpl));
    ...
    function testVerifyL1ContractsFailInvalidL1CrossDomainMessenger() public {
        ...
        mockProxyAdmin.setImplementation(
            address(l1CrossDomainMessengerProxy), address(differentL1CrossDomainMessengerImpl)
        );
    ...
    function testVerifyL1ContractsFailInvalidOptimismPortal() public {
        ...
        mockProxyAdmin.setImplementation(address(optimismPortalProxy), address(differentOptimismPortalImpl));
    ...
    function testVerifyAndRegisterRollupConfigFailInvalidNativeToken() public {
        ...
        // Set owner to thirdOwner after re-initialization
        MockSystemConfig(address(systemConfigProxy)).setOwner(thirdOwner);
        ...
        L1ContractVerification newVerifier = L1ContractVerification(
            address(
                new TransparentUpgradeableProxy(
                    ...
                    abi.encodeWithSelector(L1ContractVerification.initialize.selector, nativeToken, owner)
                )
            )
        );

        // Setup the new verifier as owner
        vm.startPrank(owner);
        ...
        // Grant admin role to owner and thirdOwner so they can call the function
        newVerifier.addAdmin(owner);
        newVerifier.addAdmin(thirdOwner);
        vm.stopPrank();

        // Test the verification - call as thirdOwner since thirdOwner owns the system config proxy
        vm.startPrank(thirdOwner);
    ...
    function testVerifyL1ContractsWithLargeOwnerSet() public {
        ...
        largeProxyAdmin.setImplementation(address(systemConfigProxy), address(systemConfigImpl));
        largeProxyAdmin.setImplementation(address(l1StandardBridgeProxy), address(l1StandardBridgeImpl));
        largeProxyAdmin.setImplementation(address(l1CrossDomainMessengerProxy), address(l1CrossDomainMessengerImpl));
        largeProxyAdmin.setImplementation(address(optimismPortalProxy), address(optimismPortalImpl));

        // Set admin relationships for each proxy
        largeProxyAdmin.setAdmin(address(systemConfigProxy), address(largeProxyAdmin));
        largeProxyAdmin.setAdmin(address(l1StandardBridgeProxy), address(largeProxyAdmin));
        largeProxyAdmin.setAdmin(address(l1CrossDomainMessengerProxy), address(largeProxyAdmin));
        largeProxyAdmin.setAdmin(address(optimismPortalProxy), address(largeProxyAdmin));
        ...
        // Setup all contract information
        verifier.setLogicContractInfo(address(systemConfigProxy), IProxyAdmin(address(largeProxyAdmin)));
        ...
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(largeProxyAdmin)), address(largeSafe), "Test Rollup"
        );
    ...
    function testUpgradeWithoutMultisigShouldFail() public {
        ...
        verifierProxyAdmin.upgrade(verifierProxy, address(newImplementation));
    ...
    function testUpgradeWithMultisigSuccess() public {
        ...
        // Simulate multisig wallet call - the safe wallet itself should be able to call upgrade
    ...
    function testProxyAdminOwnershipAfterDeployment() public {
        ...
        // Deploy proxy
        bytes memory initData = abi.encodeWithSelector(L1ContractVerification.initialize.selector, nativeToken, owner);

        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(implementationContract), address(verificationContractProxyAdmin), initData
        );
        ...
        vm.expectRevert("Ownable: caller is not the owner");
        verificationContractProxyAdmin.upgrade(proxy, address(newImplementation));
    ...
    function testMultipleOperatorSafeWallets() public {
        ...
        // Set safe wallet configurations for first safe wallet
        verifier.setSafeConfig(
            tokamakDAO, // _tokamakDAO
            foundation, // _foundation
            3, // _threshold
        ...
        // Test verification with first safe wallet - call as thirdOwner since thirdOwner owns the system config proxy
        vm.startPrank(owner);
        // Set safe wallet configuration for safe1
        verifier.setSafeConfig(
            tokamakDAO, // _tokamakDAO
            foundation, // _foundation
            3, // _threshold
        ...
        vm.startPrank(thirdOwner);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(proxyAdmin1)), address(safe1), "Test Rollup"
        );

        // Test verification with second safe wallet - call as thirdOwner since thirdOwner owns the system config proxy
        vm.startPrank(owner);
        // Set safe wallet configuration for safe2
        verifier.setSafeConfig(
            tokamakDAO, // _tokamakDAO
            foundation, // _foundation
            3, // _threshold
        ...
        // Update mockProxyAdmin to be owned by safe2 for this test
        vm.startPrank(owner);
        mockProxyAdmin.setOwner(address(safe2));
        vm.stopPrank();

        // Transfer ownership of SystemConfig to the operator of safe2
        vm.startPrank(thirdOwner);
        MockSystemConfig(address(systemConfigProxy)).setOwner(makeAddr("differentOwner"));
        vm.stopPrank();

        vm.startPrank(makeAddr("differentOwner"));
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)), address(safe2), "Test Rollup 2"
        );
        vm.stopPrank();

        // Now test with the second proxy admin by setting up a new system config proxy owned by it
        vm.startPrank(owner);

        // Create a new system config proxy owned by proxyAdmin2
        TransparentUpgradeableProxy systemConfigProxy2 = new TransparentUpgradeableProxy(
            address(systemConfigImpl),
            address(proxyAdmin2),
            abi.encodeWithSelector(
                MockSystemConfig.initialize.selector,
        ...
        // Set up implementations for proxyAdmin2 to match the new systemConfigProxy2
        proxyAdmin2.setImplementation(address(systemConfigProxy2), address(systemConfigImpl));
        proxyAdmin2.setAdmin(address(systemConfigProxy2), address(proxyAdmin2));

        // Set up the logic contract info for the new proxy
        verifier.setLogicContractInfo(address(systemConfigProxy2), IProxyAdmin(address(proxyAdmin2)));

        vm.stopPrank();

        // Now test that thirdOwner cannot use the second proxy admin with the second system config proxy
        vm.startPrank(thirdOwner);
        vm.expectRevert(L1ContractVerification.CallerNotDeployer.selector);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy2), IProxyAdmin(address(proxyAdmin2)), address(safe2), "Test Rollup 3"
        );
        vm.stopPrank();
    }
    ...
    function testVerifyL1ContractsFailWithModules() public {
        ...
        safeProxyAdmin.setImplementation(address(systemConfigProxy), address(systemConfigImpl));
        safeProxyAdmin.setImplementation(address(l1StandardBridgeProxy), address(l1StandardBridgeImpl));
        safeProxyAdmin.setImplementation(address(l1CrossDomainMessengerProxy), address(l1CrossDomainMessengerImpl));
        safeProxyAdmin.setImplementation(address(optimismPortalProxy), address(optimismPortalImpl));
        ...
        verifier.setSafeConfig(
            tokamakDAO, foundation, 3, maliciousSafe.masterCopy().codehash, address(maliciousSafe).codehash
        );
        ...
        // Test that verification fails when modules are present - call as thirdOwner since thirdOwner owns the system
        // config proxy
        vm.startPrank(thirdOwner);
        vm.expectRevert(L1ContractVerification.SafeWalletUnauthorizedModules.selector);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(safeProxyAdmin)), address(maliciousSafe), "Test Rollup"
        );
        vm.stopPrank();
    }
    ...
    function testVerifyL1ContractsFailContractCall() public {
        ...
        // Setup safe wallet info
        verifier.setSafeConfig(tokamakDAO, foundation, 3, implementationCodehash, proxyCodehash);
        ...
        // Call verify as the thirdOwner - this should succeed since thirdOwner owns the system config proxy
        vm.startPrank(thirdOwner);
        verifier.verifyAndRegisterRollupConfig(
            address(systemConfigProxy), IProxyAdmin(address(mockProxyAdmin)), address(safeWallet), "Test Rollup"
        );
