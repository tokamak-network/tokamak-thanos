...
contract L1ContractVerificationMultisigConfigurationCalls is Script {
    ...
    function run() public {
        ...
        uint256 txIndexSetSafeConfig = multiSig.getTransactionCount() - 1;
        console.log("setSafeConfig transaction submitted by owner 1, txIndex:", txIndexSetSafeConfig);
        ...
        bytes memory setLogicContractInfoData = abi.encodeWithSelector(
            L1ContractVerification.setLogicContractInfo.selector, systemConfigProxyAddress, proxyAdminAddress
        );

        vm.broadcast(_multisigOwner1);
        multiSig.submitTransaction(_proxyAddress, 0, setLogicContractInfoData);
        uint256 txIndexSetLogic = multiSig.getTransactionCount() - 1;
        console.log("setLogicContractInfo transaction submitted by owner 1, txIndex:", txIndexSetLogic);
