...
contract DeployL2OutputOracle is Script {
    ...
    function run() external {
        ...
        uint256 l2OutputOracleSubmissionInterval = 120;
        uint256 l2BlockTime = 6;
        uint256 l2OutputOracleStartingBlockNumber = 1735700;
        uint256 l2OutputOracleStartingTimestamp = 1732843488;
        address l2OutputOracleProposer = address(0xA24E06AF98029DA5120c1A93AdF260bcdB491E7b);
        address l2OutputOracleChallenger = address(0xC9402d34Cd9B375d1eD19c92c9Ac325738301FA5);
        uint256 finalizationPeriodSeconds = 12;

        // start broadcast
        uint256 deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
        vm.startBroadcast(deployerPrivateKey);

        // deploy new L2OutputOracle
        ...
        // _upgradeViaSafe({
        //     _proxy: payable(l2OutputOracleProxy),
        //     _implementation: address(0x15AF90E59a1e7ec8f133ede1322eb966BbC97A66)
        // });
    ...
    function _upgradeAndCallViaSafe(address _proxy, address _implementation, bytes memory _innerCallData) internal {
       ...
       /// @notice Call from the Safe contract to the Proxy Admin's upgrade and call method
    function _upgradeViaSafe(address _proxy, address _implementation, bytes memory _innerCallData) internal {
        address proxyAdmin = vm.envAddress("PROXY_ADMIN_ADDRESS");
        address systemOwnerSafe = vm.envAddress("SYSTEM_OWNER_SAFE_ADDRESS");
        bytes memory data =
    ...
    function _appendDeployment(string memory _name, address _deployed) internal {
        _deploymentOutfile = Config.deploymentOutfile();
        console.log("Writing artifact to %s", _deploymentOutfile);
        ForgeArtifacts.ensurePath(_deploymentOutfile);

        vm.writeJson({ json: stdJson.serialize("", _name, _deployed), path: _deploymentOutfile });
    }
}
