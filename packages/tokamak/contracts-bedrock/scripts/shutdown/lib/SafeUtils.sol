...
library SafeUtils {
  ...
  function execViaSafe(
    ...
    return execViaSafeWithNonce(safe, target, data, label, callerPrivateKey, 0);
  }

  /// @notice Execute transaction via Gnosis Safe with specific nonce offset
  function execViaSafeWithNonce(
    IGnosisSafe safe,
    address target,
    bytes memory data,
    string memory label,
    ...
    if (threshold != 1) {
      console.log('Safe execution skipped (threshold > 1) for:', label);
      return false;
    }

    if (!safe.isOwner(caller)) {
      console.log('Safe execution skipped (caller not owner) for:', label);
      return false;
    }
  ...
  /// @notice Execute transaction via Safe using caller address (from env) with nonce offset
  function execViaSafeFromEnvWithNonce(
    IGnosisSafe safe,
    address target,
    bytes memory data,
    string memory label,
    ...
    uint256 privateKey = Vm(VM_ADDRESS).envUint('PRIVATE_KEY');
    return
      execViaSafeWithNonce(safe, target, data, label, privateKey, nonceOffset);
  }
  ...
  function logSafeTx(
    ...
    logSafeTxWithNonce(safe, target, data, label, 0);
  }

  /// @notice Log Safe transaction details with specific nonce offset
  function logSafeTxWithNonce(
    IGnosisSafe safe,
    address target,
    bytes memory data,
    string memory label,
    ...
    uint256 nonce = safe.nonce() + nonceOffset;
  ...
  function execWithOwner(
    ...
    if (_isContract(ownerToUse)) {
      ...
      if (success) {
        ...
        console.log('[WARN] Safe execution skipped. Manual signing required.');
      }
    } else {
